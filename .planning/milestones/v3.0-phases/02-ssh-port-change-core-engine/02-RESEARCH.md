...
```bash
cd gui-pro && npm install class-variance-authority tailwind-merge clsx
```

**Примечание:** `clsx` часто идёт в паре с `tailwind-merge` в виде утилиты `cn()`:

```ts
// gui-pro/src/shared/lib/cn.ts (создать в Wave 0)
...
1. `npm install class-variance-authority tailwind-merge clsx` в gui-pro
2. Создать `gui-pro/src/shared/lib/cn.ts`
...
| Config file | `gui-pro/vitest.config.ts` (или в vite.config.ts) |
| Quick run | `cd gui-pro && npm test` |
| Full suite | `cd gui-pro && npm run prerelease` (typecheck + lint + test + rust:check + build) |

### Phase Requirements → Test Map

| Req ID | Поведение | Тип теста | Команда | Файл существует? |
|--------|----------|-----------|---------|-----------------|
| COMP-01 | Zero hardcoded colors | Ручная проверка (Storybook visual) + grep | `grep -r "rgba\|#[0-9a-f]\{3,6\}" gui-pro/src/shared/ui/` | N/A |
| COMP-02 | Button CVA variants рендерятся | unit | `cd gui-pro && npm test -- Button` | Button.test.tsx (нужно обновить) |
| COMP-03 | Badge CVA variants рендерятся | unit | `cd gui-pro && npm test -- Badge` | Badge.test.tsx (нужно обновить) |
| COMP-04 | Input clearable, helper text | unit | `cd gui-pro && npm test -- Input` | Input.test.tsx (нужно создать/проверить) |
| COMP-05 | Select кастомный, keyboard nav | unit | `cd gui-pro && npm test -- Select` | Select.test.tsx (нужно обновить) |
| COMP-06 | Modal size variants | unit | `cd gui-pro && npm test -- Modal` | Modal.test.tsx (нужно обновить) |
| COMP-07-14 | Новые компоненты рендерятся | unit | `cd gui-pro && npm test` | ❌ создать Wave 3 |
| SB-04, SB-05 | Stories рендерятся в Storybook | визуальная проверка | `cd gui-pro && npm run storybook` | ❌ создать Wave 4 |

### Sampling Rate

- **После каждого компонента:** `cd gui-pro && npm test`
- **После каждой волны:** `cd gui-pro && npm run typecheck && npm run lint && npm test`
- **Phase gate:** полный `npm run prerelease` + ручной просмотр в Storybook (light + dark)

### Wave 0 Gaps

- [ ] `gui-pro/src/shared/lib/cn.ts` — утилита cn() для CVA + tailwind-merge
- [ ] `gui-pro/node_modules/class-variance-authority` — `npm install class-variance-authority tailwind-merge clsx`
...
- `gui-pro/src/shared/ui/*.tsx` — все 19 существующих компонентов прочитаны напрямую
- `gui-pro/src/shared/styles/tokens.css` — 263 строки, все токены проверены
...
- `gui-pro/package.json` — все зависимости проверены
- `gui-pro/.storybook/main.ts` + `preview.ts` — Storybook конфигурация
...
- `gui-pro/tailwind.config.js` — `surface.*` palette в extend.colors (подтверждает DS-10 требование)
- `gui-pro/src/shared/ui/*.test.tsx` — Badge.test.tsx и Button.test.tsx прочитаны для понимания scope
