...
Sanitize: функция находится в Rust (`gui-pro/src-tauri/src/logging.rs`), уже использует loop-based multi-occurrence поиск — судя по тестам, она **уже корректна** для нескольких вхождений. Перед включением этого таска в план необходима верификация.
...
1. **D-18 Sanitize fix:** запустить `cargo test sanitize_replaces_all_occurrences` в `gui-pro/src-tauri/` — если PASS, задача уже выполнена.
...
3. **D-12 Button variants:** запустить `npx tsc --noEmit` в `gui-pro/` — TypeScript покажет все невалидные variants.
...
| Config file | `gui-pro/vite.config.ts` (строки test:{ globals: true, environment: "jsdom" }) |
| Quick run | `cd gui-pro && npx vitest run src/components/layout/` |
| Full suite | `cd gui-pro && npx vitest run` |
| Rust tests | `cd gui-pro/src-tauri && cargo test` |
...
| D-12 | Button variants TypeScript | type-check | `npx tsc --noEmit` в gui-pro/ | ✅ |
...
- [ ] `gui-pro/src/components/ServerSidebar.test.tsx` — тест `servers.length < 2 → sidebar hidden`
- [ ] `gui-pro/src/components/ServerTabs.test.tsx` — тест state preservation при переключении табов
...
- `gui-pro/src/components/layout/TabNavigation.tsx` — полное чтение
- `gui-pro/src/components/ServerSidebar.tsx` — полное чтение
- `gui-pro/src/App.tsx` — полное чтение
- `gui-pro/src-tauri/src/logging.rs` — строки 1-60, 260-320
- `gui-pro/src/shared/ui/Button.tsx` — CVA variants
- `gui-pro/src/shared/ui/Badge.tsx` — CVA variants
- `gui-pro/src/shared/ui/StatusBadge.tsx` — defaultLabels
- `gui-pro/src/shared/ui/Select.tsx` — hardcoded placeholder
- `gui-pro/src/shared/ui/EmptyState.tsx` — hardcoded defaults
- `gui-pro/src/shared/i18n/locales/ru.json` — existing keys
- `gui-pro/src/shared/i18n/locales/en.json` — existing keys
- `gui-pro/src/shared/styles/tokens.css` — transition + sidebar tokens
- `gui-pro/src/components/ServerTabs.tsx` — conditional render bug
