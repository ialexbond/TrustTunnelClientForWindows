...
**Root cause:** [`useServerStats.ts:115-125`](gui-pro/src/components/server/useServerStats.ts:115) — first fire запланирован через `intervalMs` (default 10s), не immediate. Initial state: `stats=null, loading=false`. В OverviewSection ([`OverviewSection.tsx:416,429`](gui-pro/src/components/server/OverviewSection.tsx:416)) условие `stats === null && statsLoading` — оба false первые 10s, показывает «—».
...
**Подтверждение что это статус протокола:** [`OverviewSection.tsx:248`](gui-pro/src/components/server/OverviewSection.tsx:248) — `isRunning = serverInfo.serviceActive`. Это поле возвращается из `systemctl is-active trusttunnel.service` через SSH. То есть карточка отражает живость **VPN-сервиса** (демона), а не сервера в целом.

**Fix:** переименовать i18n-ключ `server.overview.cards.status` ([`ru.json:987`](gui-pro/src/shared/i18n/locales/ru.json:987)):
...
**Root cause:** [`OverviewSection.tsx:386-388`](gui-pro/src/components/server/OverviewSection.tsx:386) — для firewall/fail2ban/sshKey hardcoded `ok: null`, что даёт `bg = bg-elevated, color = text-muted`. Проверка реально только для TLS (через `state.serverInfo`).
...
**Нет, реальное измерение.** Backend [`network.rs:93-115`](gui-pro/src-tauri/src/commands/network.rs:93) — `ping_endpoint(host, port=443)`:
...
- Раньше тот же экран мог использовать `health_check` ([`network.rs:60`](gui-pro/src-tauri/src/commands/network.rs:60)) — это **HTTP GET /_check через HTTPS** с полным TLS handshake. Это 100-500ms даже на быстром сервере.
...
**Intentional descope в Phase 13.** Backend `speedtest_run` существует и работает (вызывается в [`useDashboardState.ts:134`](gui-pro/src/components/dashboard/useDashboardState.ts:134)). Но в OverviewSection ([`OverviewSection.tsx:307`](gui-pro/src/components/server/OverviewSection.tsx:307)) кнопка refresh у Speed-карточки имеет `onRefresh={() => {}}` (no-op) и плейсхолдер «Не измерялась».
...
**Да.** Backend `server_get_stats` возвращает `uptime_seconds` из `/proc/uptime` через SSH. Frontend форматирует через `formatServerUptime(seconds, t)` ([`uptime.ts`](gui-pro/src/shared/utils/uptime.ts)). Можешь сверить с `uptime` командой в SSH-сессии.
