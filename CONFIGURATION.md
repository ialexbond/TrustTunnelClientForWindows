...
Все пути указаны относительно редакции **Pro** (`gui-app/`); в Light (`gui-light/`) структура идентична.
...
Файл лежит рядом с `.exe` (portable data dir, `gui-app/src-tauri/src/ssh/mod.rs::portable_data_dir`). Автоопределяется по наличию секции `[endpoint]` или `[listener]` (`commands/config.rs::auto_detect_config`).
...
- Инициализация: `gui-app/src/shared/i18n/index.ts`
...
- Хук: `gui-app/src/shared/hooks/useTheme.ts`
...
- Проверка — `gui-app/src/shared/hooks/useUpdateChecker.ts`:
