...
```
TrustTunnelClient/
├── gui-app/              # Pro-редакция (React + Rust + SSH)
...
Rust-код живёт в `gui-app/src-tauri/src/`. Изменения триггерят пересборку `cargo build`
внутри `tauri dev`.

```bash
# Из gui-app/src-tauri/
...
1. Добавьте ключ в `gui-app/src/shared/i18n/locales/ru.json` (приоритет — русский).
...
```rust
// gui-app/src-tauri/src/commands/network.rs
#[tauri::command]
