...
Когда связь с сервером пропала по-настоящему, а не моргнула, приложение сперва один раз пробует
переподключиться к нему же, и только потом переходит к следующему серверу в вашем списке. Порядок
вы задаёте сами, а ненужный сервер можно исключить из очереди, не удаляя его из списка.
