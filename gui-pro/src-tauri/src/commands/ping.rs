...
/// The server's DIAL IP as an `IpAddr`, read straight from a config file — the destination the
/// egress guard asks Windows to route towards (`net_egress::decide`).
///
/// Reuses the same `[endpoint]` reader the reachability probe uses, so "which address do we dial"
/// has ONE definition. Returns `None` when the config is unreadable/unparseable, or when the
/// endpoint carries a HOSTNAME rather than a literal IP: resolving it here would (a) duplicate the
/// core's own resolution and (b) on a machine whose DNS is poisoned by the very adapter we are
/// trying to route around, resolution could itself be wrong. Configs generated by this app always
/// dial a bare IP in `addresses[0]`, so the hostname case is the rare one — and there the guard
/// simply makes no override, leaving behaviour exactly as it is today.
pub(crate) fn config_dial_ip(config_path: &str) -> Option<std::net::IpAddr> {
    let content = std::fs::read_to_string(config_path).ok()?;
    let (host, _port) = read_endpoint_host_port(&content).ok()?;
