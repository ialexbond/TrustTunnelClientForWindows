...
async fn deploy_export_config(
    ...
    // ── gap 5b: make the install config cert-type-aware ─────────────────────
    //
    // build_client_config is cert-type-blind — it ships skip_verification
    // absent + certificate="" + no custom_sni, which means the OS trust store
    // must validate the endpoint cert. That is correct for Let's-Encrypt but
    // FATAL for a self-signed / operator-provided cert: the OS store rejects
    // it → the TLS handshake fails → the app is stuck on «Переключение».
    //
    // The confirmed-working self-hosted config (owner build q4m8xt) is exactly
    // the generated config PLUS three [endpoint] fields:
    //   skip_verification = true, custom_sni = domain-or-"trusttunnel.local",
    //   certificate = """<self-signed leaf PEM>""".
    // We derive the SNI the same way the working case had it (no real
    // domain ⇒ "trusttunnel.local"), probe the just-installed endpoint to pin
    // its leaf cert, and apply the policy via the shared helper that also backs
    // the Users-tab fetch path (single source of truth).
    //
    // fetch_endpoint_cert REJECTS an IP SNI (rustls won't send SNI for an IP),
    // so always pass this domain-or-.local SNI, never a bare IP. The TCP
    // destination CAN be an IP — that is params.host / settings.domain.
    let sni = if settings.domain.is_empty() {
        "trusttunnel.local".to_string()
    } else {
        settings.domain.clone()
    };
...
mod tests {
    ...
    // ─── gap 5b: cert-type-aware install policy (pure derivation) ─────────────
    //
    // deploy_export_config derives the SNI (empty domain ⇒ "trusttunnel.local")
    // then feeds cert_type + that SNI + the probed PEM to
    // install_cert_policy_for. These tests pin the pure derivation for both
    // branches + the empty-domain rule without SSH or the async probe.

    use crate::ssh::server::install_cert_policy_for;

    /// Mirror of deploy_export_config's SNI derivation, tested in isolation.
    fn derive_sni(domain: &str) -> String {
        if domain.is_empty() { "trusttunnel.local".to_string() } else { domain.to_string() }
    }

    #[test]
    fn install_cert_policy_for_self_hosted_no_domain_uses_local_sni() {
        // Owner's confirmed self-hosted case: no real domain ⇒ SNI "trusttunnel.local",
    ...
    #[test]
    fn install_cert_policy_for_self_hosted_probe_failure_degrades_to_skip_only() {
