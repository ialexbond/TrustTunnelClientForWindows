/**
 * ServiceTabSection.test.tsx — Phase 17 Plan 06
 *
 * Tests:
...
// Phase 19 Plan 19-04 — ProtocolUpdateSection is the 4th block. Mock keeps
// tests isolated from useSidecarVersions Tauri calls / Modal portal.
vi.mock("./ProtocolUpdateSection", () => ({
  ProtocolUpdateSection: () => (
    <div data-testid="protocol-update-section-card">ProtocolUpdateSection</div>
  ),
}));
...
describe("ServiceTabSection", () => {
  ...
  it("renders_6_blocks_in_canonical_order — BBR → MTProto → Benchmark → ProtocolUpdate → Logs → DangerZone (Phase 19 Plan 19-04)", () => {
    ...
    // All 5 section testids present (BBR, MTProto, Benchmark, ProtocolUpdate, Logs)
    ...
    const protocolUpdateCard = document.querySelector(
      '[data-testid="protocol-update-section-card"]',
    );
  ...
  it("restart_button_not_in_service (W5 explicit anti-presence) — overview-restart-service-button MUST NOT be in Utilities", () => {
    ...
    // Also verify no legacy service-level restart button
  ...
  it("no_handleStopService_local — ServiceTabSection renders without Stop/Restart logic at orchestrator level", () => {
    ...
    // Stop button test-id must NOT appear at the service orchestrator level.
    render(<ServiceTabSection state={makeState()} />);
    // The service orchestrator does NOT directly render a Stop or Restart button.
